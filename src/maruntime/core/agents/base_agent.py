...
from maruntime.runtime.session_service import ChatMessage, MessageStore, SessionContext, SessionService
from maruntime.runtime.templates import LLMPolicy, TemplateRuntimeConfig, ToolPolicy
...
class BaseAgent(AgentRegistryMixin, abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        # Use prompts from template_config if available, otherwise use provided or defaults
        if prompts_config is not None:
            self.prompts_config = prompts_config
        elif template_config is not None:
            self.prompts_config = template_config.get_prompts_config()
        else:
    ...
    def _clarification_message(self, clarifications: str) -> str:
        """Format clarification response using the configured template."""
        return PromptLoader.get_clarification_template(clarifications, self.prompts_config)

    async def provide_clarification(self, clarifications: str) -> None:
        """Receive clarification from user and add to conversation.
        
        This method is called when the user responds to a ClarificationTool request.
        It formats the clarification using the configured template and records it
        as a user message in the session.
        
        Args:
            clarifications: The user's clarification response text
        """
        await self._ensure_session_state()
        
        # Format clarification using template
        formatted = self._clarification_message(clarifications)
        
        # Record as user message
        await self._record_message(ChatMessage.text("user", formatted))
        
        # Update context
        self._context_data["clarifications_used"] = self._context_data.get("clarifications_used", 0) + 1
        self._context_data["last_clarification"] = clarifications
        
        # Update session state if service available
