"""Tool for extracting full content from web pages."""
...
class ExtractPageContentTool(PydanticTool):
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        content_limit = config.get("content_limit", 5000) if config else 5000

        sources = await search_service.extract(urls=self.urls)
        ...
        for url in self.urls:
            if url in context.sources:
                ...
                if source.full_content:
                    content_preview = source.full_content[:content_limit]
                    formatted_result += (
                        f"{str(source)}\n\n**Full Content:**\n"
                        f"{content_preview}\n\n"
...
__all__ = ["ExtractPageContentTool"]
